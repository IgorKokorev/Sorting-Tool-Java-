...
public class Main {
    ...
    public static void main(final String[] args) throws IOException {
        ...
        if (outFilePos >= 0) {
            if (outFilePos < (argList.size() - 1)) {
                outFile = new File(argList.get(outFilePos + 1));
        ...
        scanner.close();
        if (outFile != null ) myWriter.close();
    }

    // Sort and print depending on sorting type
    private static <T extends Comparable<T>> void processData(ArrayList<T> strings, ArrayList<MyPair<T>> stringsMap) throws IOException {
        if (isNaturalSorting) {
    ...
    // print map sorting by count
    private static <T extends Comparable> void printByCount(ArrayList<MyPair<T>> map, int size) throws IOException {
        myPrintln("Total " + inpType + "s: " + size + ".");
        for (MyPair<T> pair : map) {
            myPrintln(pair.key + ": " + pair.num + " time(s), " + (pair.num * 100 / size) + "%");
        }
    }
    ...
    // print natural sorting list
    private static <T> void printNatural(ArrayList<T> list) {
        myPrintln("Total " + inpType + "s: " + list.size() + ".");
        myPrint("Sorted data:");
        for (T elem : list) myPrint(separator + elem);
        myPrintln("");
    }

    private static void myPrint(String s) {
        if (outFile == null) System.out.print(s);
        else {
            try {
                myWriter.write(s);
            } catch (IOException e) {
